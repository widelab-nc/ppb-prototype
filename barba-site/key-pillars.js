...
(function () {
  /* ⚠️ BUG NAPRAWIONY 2026-07-29 (audyt): `var PPB` stało na DOLE tego IIFE
     (przy rejestracji kontraktu). Hoisting var robi z lokalnego PPB `undefined`
     w tym miejscu → `PPB.config` rzucało TypeError, CAŁY plik padał na starcie
     i PPB.sections.keyPillars nigdy się nie rejestrowało (key pillars / concept /
     CTA / footer-bottle-reveal martwe na home, w każdej przeglądarce).
     Kanon nie ma tej deklaracji w ogóle; kopia dostała ją przy dopisywaniu
     kontraktu Barby. Deklaracja MUSI być przed pierwszym użyciem — czyli tutaj. */
  ...
  const CONFIG = {
    ...
    concept: {
      ...
      /* MOBILE≤991 (2026-07-28) — artboard 375: node 4473:6327.
         Ta sama choreografia, INNA OŚ: na desktopie kształty rozjeżdżają się w POZIOMIE,
         na mobile w PIONIE (kwadrat w GÓRĘ, koło w DÓŁ). Na 375 px poziom nie ma dokąd
         się rozjechać — pion ma 676.

         Liczby wprost z artboardu, NIE z przeskalowania desktopu: środek kwadratu
         y=371, środek koła y=499,86 → rozjazd 128,86 px = ±64,43 = ±4,027rem.
         Przeskalowany desktop dałby ±6,64rem, czyli więcej. Konsekwencja: na mobile
         kształty kończą NACHODZĄC na siebie (66,7 px zakładki), a nie z prześwitem
         42 px jak na desktopie — dokładnie jak na artboardzie. Gdyby miał być
         prześwit, to jedyne dwie liczby do ruszenia w tym bloku.

         Rozmiary (CSS) = równe 40 % desktopu: kwadrat 500→200, koło 478→191,2.
         Dzięki tej samej skali koło dalej wypełnia otwór kwadratu co do piksela
         w stanie złączonym (start).

         WERSJA 2 (2026-07-28, decyzja Tomka po obejrzeniu): **kwadrat STOI**, rusza się
         tylko koło — w dół, aż między krawędziami zostanie `gapRem` prześwitu.
         Dystans NIE jest tu stałą, bo zależy od rozmiarów obu kształtów, a te są
         w rem i rosną z drabinką. Liczy go `circleTravel()` z REALNYCH boksów:
             droga = (szer.kwadratu + szer.koła) / 2 + gap
         (start jest koncentryczny, więc odległość środków = suma promieni + prześwit).
         Dzięki temu zmiana rozmiaru kształtów w CSS-ie — np. mniejsze w poziomie
         telefonu — automatycznie poprawia dystans, bez drugiej liczby do pilnowania. */
      mobile: {
        gapRem: 1,                  // prześwit między krawędziami na KOŃCU (1rem)
        /* KONIEC WCZEŚNIEJ NIŻ NA DESKTOPIE (2026-07-28, decyzja Tomka).
           "bottom bottom+=15%" = okno zamyka się, gdy dół sekcji jest jeszcze 15 %
           wysokości ekranu PONIŻEJ dolnej krawędzi viewportu — czyli koło dojeżdża
           na miejsce, zanim sekcja zacznie wychodzić górą. Desktop zostaje na
           "bottom bottom". */
        shapesEnd: "bottom bottom+=15%",
      },
    },
  ...
  function initKeyPillars() {
    ...
    const section = ROOT.querySelector(".is-home-key-pillars");
    if (!section) return;
    ...
    /* ============================================================
       MOBILE≤991 — ŁAMANIA NAGŁÓWKÓW: `<br>` USUWANY Z DOM-U, nie chowany CSS-em
       (2026-07-28, zgłoszenie Tomka: „tekst łamie się niepotrzebnie, «full» jest samo").

       Pierwsze podejście chowało znaczniki regułą `.home-key-pillars_heading br
       { display: none }`. NIE DZIAŁA: SplitText łamie linię na `<br>` niezależnie od
       jego `display` — dowód pomiarowy, nagłówek „A foundation <br>for reach" po splicie
       ma 2 linie, mimo że wizualnie mieści się w jednej. Efekt na ekranie był jeszcze
       gorszy dla „One point, <br>full orchestration": SplitText robił linię
       „full orchestration", która nie mieściła się w 335 px i zawijała jeszcze raz —
       stąd „full" samo w wierszu.

       Znacznik musi więc zniknąć ZANIM ktokolwiek zrobi split (a split leci leniwie,
       przy pierwszym `revealText` danego stanu). Podmieniamy na spację, nie usuwamy:
       w markupie jest „A clean track <br>record", ale gdyby ktoś kiedyś wpisał wariant
       bez spacji, samo usunięcie skleiłoby słowa.
       ============================================================ */
    if (mobile) {
      section.querySelectorAll(".home-key-pillars_heading br").forEach(function (br) {
    ...
    /* ============================================================
       MOBILE≤991 — WYSOKOŚĆ PUDEŁKA COPY LICZONA Z NAJWYŻSZEGO STANU
       (2026-07-28, zgłoszenie Tomka: „tekst nachodzi na baton — liczmy miejsce
       od największego punktu, bo one stoją w tym samym miejscu, a mają różną wysokość").

       Cztery stany są stackowane absolutnie w jednym slocie, więc slot musi być tak
       wysoki jak NAJWYŻSZY z nich — inaczej dłuższy pillar wychodzi poza pudełko
       i wchodzi na CTA. Wcześniej ta liczba była stałą w CSS (`--kp-m-copy-h: 11rem`),
       oszacowaną z pomiaru przed splitem. To było kruche z trzech powodów:
         (a) SplitText ZAPIEKA łamania linii przy pierwszym reveal — wysokość po
             splicie potrafi być inna niż przed;
         (b) w paśmie 375–991 rośnie i root (16→19), i szerokość kolumny, więc liczba
             linii zmienia się nieliniowo — jedna stała nie opisuje całego pasma;
         (c) każda zmiana copy w Webflow unieważnia oszacowanie po cichu.

       Teraz mierzymy realnie renderowane stany i zapisujemy px w inline stylu.
       Wołane na `refreshInit`, czyli ZANIM ScrollTrigger policzy pozycje — dzięki temu
       zmiana wysokości slotu nie unieważnia świeżo policzonych startów i endów.
       CSS-owy `--kp-m-copy-h` zostaje jako wartość startowa (zero FOUC, zanim JS zmierzy).
       ============================================================ */
    /* ============================================================
       MOBILE≤991 — WYSOKOŚĆ SEKCJI Z CONFIGU, nie z drugiej kopii liczby w CSS
       (2026-07-28). Bez GSAP-owego pinu nie ma spacera, więc sekcja musi mieć jawną
       wysokość. Sticky trzyma przez `wysokość − 100vh`, a okno scruba to
       `pinViewports × 100vh` — żeby jedno równało się drugiemu, sekcja potrzebuje
       `(pinViewports + 1) × 100vh`. Ta liczba MUSI wynikać z `c.pinViewports`,
       bo inaczej jest wpisana w dwóch miejscach i po zmianie configu sticky odkleja
       się przed końcem scruba (albo zostaje pusty ogon). Ten sam wzorzec co
       how-it-works v4 w script.js. */
    function applySectionHeight() {
      const m = window.matchMedia("(max-width: 991px)").matches;
      section.style.height = m ? ((c.pinViewports + 1) * 100) + "vh" : "";
    }
    applySectionHeight();
    ScrollTrigger.addEventListener("refreshInit", applySectionHeight);

    const copyBox = section.querySelector(".home-key-pillars_copy");
    function fitCopyBox() {
      if (!window.matchMedia("(max-width: 991px)").matches) {
        copyBox.style.height = "";
        pin.style.removeProperty("--kp-m-copy-h");
        return;
      }
      let max = 0;
      copies.forEach(function (el) {
        const prevPos = el.style.position, prevVis = el.style.visibility;
        // pomiar wymaga wyjęcia stanu ze stosu absolutnego; visibility zostawiamy
        // (element ukryty przez `visibility` NADAL ma layout, więc mierzy się poprawnie)
        el.style.position = "static";
        max = Math.max(max, el.getBoundingClientRect().height);
        el.style.position = prevPos; el.style.visibility = prevVis;
      });
      if (!max) return;
      const px = Math.ceil(max);
      copyBox.style.height = px + "px";
      /* JEDNO ŹRÓDŁO PRAWDY. Budżet wysokości ringu (`--kp-m-stage` w style.css) odejmuje
         od 100vh m.in. wysokość pudełka copy. Gdyby ring liczył ją z CSS-owego
         oszacowania, a pudełko z pomiaru, to przy dłuższym tekście ring zabierałby
         miejsce, którego już nie ma, i komponent wychodziłby poza sticky.
         Inline na elemencie bije regułę z arkusza (także tę z bloku poziomego),
         więc CSS-owe `--kp-m-copy-h` zostaje wyłącznie wartością startową na czas
         przed pierwszym pomiarem. */
      pin.style.setProperty("--kp-m-copy-h", px + "px");
    }
    fitCopyBox();
    ScrollTrigger.addEventListener("refreshInit", fitCopyBox);
    ...
    /* MOBILE≤991 (2026-07-28): wideo stoi na CSS-owym STICKY, nie na GSAP-owym pinie.
       Zasada z HANDOFF-mobile-2: „Nieruchomości elementu NIGDY nie udajemy JS-em" —
       pin pozycjonuje transformem pisanym co zdarzenie scrolla, sticky robi przeglądarka.
       Warunek poprawności: `.section.is-home-key-pillars` musi mieć na mobile jawne
       500vh = (pinViewports + 1) × 100vh (style.css, blok KEY PILLARS), bo bez pinu
       GSAP nie dokłada już spacera i sekcja nie miałaby z czego trzymać sticky.
       Okno triggera zostaje identyczne (top top → +=400%), więc reszta choreografii
       (wideo, pierścień, swap copy) liczy się TAK SAMO na obu warstwach. */
    const st = ScrollTrigger.create({
      trigger: section,
      start: "top top",
      end: `+=${c.pinViewports * 100}%`,
      pin: mobile ? false : pin,
      scrub: c.scrubSmooth,
      onUpdate: (self) => {
  ...
  function initConcept() {
    ...
    const section = ROOT.querySelector(".is-home-concept");
    if (!section) return;
    ...
    /* MOBILE≤991 — WYSOKOŚĆ COPY DO CSS-a (2026-07-28).
       `--concept-sq` liczy rozmiar kształtów z miejsca, które ZOSTAJE pod tekstem,
       żeby dolny odstęp równał się górnemu. CSS nie umie zmierzyć tekstu, a wysokość
       nagłówka zmienia się z liczbą linii (na 375 są trzy, od 768 dwie), więc jedyną
       uczciwą drogą jest pomiar. Na `refreshInit`, czyli zanim ScrollTrigger policzy
       pozycje — inaczej zmiana rozmiaru kształtów unieważniałaby świeże starty i endy. */
    const conceptPin = section.querySelector(".home-concept_pin");
    const conceptCopy = section.querySelector(".home-concept_copy");
    function fitConceptCopy() {
      if (!window.matchMedia("(max-width: 991px)").matches) {
        conceptPin.style.removeProperty("--concept-copy-h");
        return;
      }
      const h = conceptCopy.getBoundingClientRect().height;
      if (h > 0) conceptPin.style.setProperty("--concept-copy-h", Math.ceil(h) + "px");
    }
    fitConceptCopy();
    ScrollTrigger.addEventListener("refreshInit", fitConceptCopy);
    ...
    /* KSZTAŁTY: zbiegają się od shapesStart i ŁĄCZĄ się na shapesEnd (bottom bottom).
       MOBILE≤991: identyczne okno i ease, zmienia się TYLKO OŚ — `y` zamiast `x`
       (patrz CONFIG.concept.mobile). Pozycję wyjściową pary daje CSS (`top` na
       kształtach), a nie transform, właśnie po to, żeby GSAP miał wolną oś do animacji
       i nie deptał offsetu startowego. */
    const mobile = window.matchMedia("(max-width: 991px)").matches;

    /* Droga koła liczona z REALNYCH boksów, nie ze stałej — patrz CONFIG.concept.mobile.
       Funkcja, a nie liczba, bo `invalidateOnRefresh` przelicza ją po każdym resize
       (rozmiary kształtów są w rem, więc zmieniają się z drabinką i z orientacją). */
    function circleTravel() {
      const sq = square.getBoundingClientRect().width;
      const ci = circle.getBoundingClientRect().width;
      const rem = parseFloat(getComputedStyle(document.documentElement).fontSize) || 16;
      return (sq + ci) / 2 + c.mobile.gapRem * rem;
    }

    if (mobile) {
      /* MOBILE≤991: kwadrat STOI, jedzie samo koło — w DÓŁ, do prześwitu 1rem.
         Kwadrat nie dostaje żadnego tweenu (nie „animujemy do zera" — po prostu
         nie ruszamy, więc nic nie może go zdryfować). */
      gsap.timeline({
        scrollTrigger: {
          trigger: section,
          start: c.shapesStart,
          end: c.mobile.shapesEnd,
          scrub: c.scrubSmooth,
          invalidateOnRefresh: true,
        },
      }).fromTo(circle, { y: 0 }, { y: circleTravel, ease: "none" }, 0);
    } else {
      gsap.timeline({
        scrollTrigger: {
          trigger: section,
  ...
  function initEntryOverlap() {
    ...
    const weKnow = ROOT.querySelector(".is-home-we-know");
    const block = ROOT.querySelector(".home-key-pillars-block");   // element NACHODZĄCY (intro + key-pillars)
    if (!weKnow || !block) return;

    /* MOBILE≤991 (2026-07-27): NA MOBILE NIE MA NACHODZENIA. style.css @991 zeruje
       margin-top bloku key-pillars, a We-know jest statycznym stackiem (nie sticky):
       overlay jest display:none, a DRYF przesuwałby w górę CAŁĄ (wysoką) kolumnę
       We-know → dziura pod sekcją. Własne matchMedia (nie isMobile ze script.js),
       żeby plik dalej był samodzielny przy porcie do Webflow — próg TEN SAM (991). */
  ...
  function initConceptNavTheme() {
    const concept = ROOT.querySelector(".is-home-concept");
  ...
  /* ============================================================
     ...
     ============================================================ */
  function initCta() {
    const section = ROOT.querySelector(".is-home-cta");
    if (!section) return;
  ...
  function initWfNavThemes() {
    ...
    /* MOBILE≤991 (2026-07-28): `.wf-home` ma `display: none` (style.css, blok
       „TYLKO PROTOTYP"). Trigger zaczepiony o element bez layoutu ma zerową wysokość
       i pozycję 0 → ScrollTrigger policzyłby to okno na SAMEJ GÓRZE strony i nav
       zmieniałby motyw w losowym miejscu scrolla. Dlatego pomijamy tę jedną parę.
       Próg 991 ten sam co reszta pliku (własne matchMedia — plik ma zostać
       samodzielny przy porcie do Webflow).
       `.is-home-cta` jest POZA `.wf-home` i widoczny, więc zostaje na obu warstwach. */
    if (!window.matchMedia("(max-width: 991px)").matches) {
      navTransition(ROOT.querySelector(".wf-home .is-join"), "light", mask, baseNav); // how(ciemna)->join(jasna): nav CIEMNY
    }
    navTransition(ROOT.querySelector(".is-home-cta"), "dark", mask, baseNav);       // meet(jasna)->cta(ciemna): nav BIALY
  }
  ...
  function applyBottleKnobs() {
    ...
    var el = ROOT.querySelector(".footer-bottle-reveal_bottle");
    if (el) {
  ...
  function initFooterBottleReveal() {
    var section = ROOT.querySelector(".is-footer-bottle-reveal");
    if (!section) return;
  ...
  function fitKeyPillarsLeft() {
    const left = ROOT.querySelector(".home-key-pillars_left");
    if (!left) return;
  ...
  /* ============================================================
     BOOT — kontrakt Barba (było: własny listener "load").
     Listener load odpaliłby się TYLKO przy pierwszym wejściu na stronę;
     po nawigacji Barbą cała ta warstwa (key pillars, concept, CTA, footer
     bottle reveal) zostałaby martwa. Teraz wywołanie robi moduł strony
     (script.js → PPB.sections.keyPillars.init), a ctx.revert() sprząta.
     ============================================================ */
  var ROOT = document;
  var ctx = null;
  var CLEANUP = [];
  ...
  function onWin(type, fn) {
    ...
    CLEANUP.push(function () { window.removeEventListener(type, fn); });
  }

  function initSection() {
    initIntro(ROOT.querySelector(".is-home-key-pillars-intro"));  // nawiasy + reveal intro
    ...
    // NAV v2 (paczka nav): styki motywów robi arbiter w nav-v2.js (data-nav-theme na sekcjach);
    // te dwa kontrolery zostają TYLKO jako fallback, gdy nav-v2 nie jest załadowany.
    if (!window.PPB || !PPB.navV2) {
    ...
    const wf = ROOT.querySelector(".wf-home");
    ...
    fitKeyPillarsLeft();
    onST("refresh", fitKeyPillarsLeft);
    onST("refresh", applyBottleKnobs);   // --bottle-h z realnego pomiaru
    let kpRaf = null;
    onWin("resize", function () {
      if (kpRaf) return;
      kpRaf = requestAnimationFrame(function () { kpRaf = null; fitKeyPillarsLeft(); applyBottleKnobs(); });
    });
    CLEANUP.push(function () { if (kpRaf) cancelAnimationFrame(kpRaf); });

    const video = ROOT.querySelector(".home-key-pillars_video");
  ...
  function init(root) {
    ROOT = root || document;
    ctx = gsap.context(function () { initSection(); }, ROOT);
  }
  function destroy() {
  ...
  /* deklaracja `var PPB` przeniesiona na GÓRĘ IIFE (patrz komentarz tam) */
  PPB.sections = PPB.sections || {};
  PPB.sections.keyPillars = { init: init, destroy: destroy };

  /* standalone (bez harnessu Barby) — zachowanie jak dawniej */
