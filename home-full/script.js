...
function computeShapeDims() {
  ...
  const s = Math.max(window.innerWidth / f.w, window.innerHeight / f.h); // cover scale
...
function showStats(setEl) {
  ...
  const tl = gsap.timeline();
  setEl.querySelectorAll(".home-how-it-works_stat").forEach((stat, i) => {
...
function hideStats(setEl) {
  ...
  const lines = setEl.querySelectorAll(".home-how-it-works_stat-line");
  const texts = setEl.querySelectorAll(".home-how-it-works_stat-label, .home-how-it-works_stat-value");
  const tl = gsap.timeline()
...
function initHowItWorks() {
  ...
  /* --- state machine statsów — REWERSYJNA (0: nic, 1: set 1, 2: set 2) --- */
  let statsState = 0;
  function setStatsState(next) {
    if (next === statsState) return;
    if (statsState === 1) hideStats(set1);
    if (statsState === 2) hideStats(set2);
  ...
  function handleProgress(p) {
    ...
    setStatsState(pc >= c.statsSwap ? 2 : pc >= c.statsIn ? 1 : 0);
    setShapeState(pc);
  ...
  const maskStartPx = c.maskStart * remPx;
...
function initWeKnow() {
  ...
  const shapeLeftX = shapeRightEdge() - scaledW / 2 - window.innerWidth / 2;
...
window.addEventListener("load", () => {
  ...
  initHowItWorks();
  initWeKnow();
  if (window.initSiteFooter) initSiteFooter();   // globalny footer (_shared/site-footer) — ostatnia sekcja strony
  initNavMask();
